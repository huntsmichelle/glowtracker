...
export default function TaskDetailClient({ task: initial }: Props) {
  ...
  const categoryColor = routine.category?.color ?? '#6B7280';
  const isCountdown   = routine.mode === 'countdown';
  ...
  async function handleComplete() {
    ...
    const { updated } = await completeTask(task.id, routine as Routine, date);
  ...
  async function handleSkip() {
    setLoading(true);
    const { updated } = await skipTask(task.id, routine as Routine);
    if (updated) setTask({ ...task, ...updated });
    setLoading(false);
    router.refresh();
  }
  ...
  async function handleSnooze() {
    setLoading(true);
    const updated = await snoozeTask(task.id, snoozeDays, task.due_date_end);
    if (updated) setTask({ ...task, ...updated });
  ...
  async function saveNotes() {
    ...
    await supabase.from('tasks').update({ notes }).eq('id', task.id);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-gray-400">{routine.category?.name ?? 'No category'}</span>
              {isCountdown && (
                <span className="text-xs text-purple-400 font-medium">Countdown</span>
              )}
            </div>
            <h1 className="text-lg font-bold text-gray-800">{routine.name}</h1>
            {isCountdown && routine.target_label && routine.target_date && (
              <p className="text-xs text-purple-500 mt-0.5">
                → {routine.target_label} · {format(parseISO(routine.target_date), 'MMM d, yyyy')}
              </p>
            )}
          </div>
        ...
        <div className="space-y-1 text-sm text-gray-600">
          <div className="flex justify-between">
            ...
            <span>
              {format(parseISO(task.due_date_start), 'MMM d')} – {format(parseISO(task.due_date_end), 'MMM d, yyyy')}
            </span>
          </div>
          {isActionable && (
            <div className="flex justify-between">
              ...
              <span className={isOverdue ? 'text-red-500 font-medium' : ''}>
                {isOverdue
                  ? `Overdue by ${differenceInDays(today(), parseISO(task.due_date_end))}d`
                  : daysUntil > 0
          ...
          {task.actual_completion_date && (
            <div className="flex justify-between">
              ...
              <span>{format(parseISO(task.actual_completion_date), 'MMM d, yyyy')}</span>
            </div>
          )}
          {task.snooze_until && (
            <div className="flex justify-between">
              <span className="text-gray-400">Snoozed until</span>
              <span>{format(parseISO(task.snooze_until), 'MMM d, yyyy')}</span>
            </div>
          )}
        </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
              ...
              {isCountdown && (
                <p className="text-xs text-purple-500">
                  Countdown routine: remaining tasks will be recalculated from your target date.
                </p>
              )}
              <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
        <h2 className="text-sm font-semibold text-gray-700">Notes <span className="text-gray-400 font-normal">(this task only)</span></h2>
        <textarea
          rows={4}
          value={notes}
